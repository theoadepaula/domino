...
caminho="\\\\SFI-024000\\Users\\othon.oliveira\\OneDrive - Agência Nacional de Águas\\Jogo\\Domino.xlsm"
...
jogadores_aposentados=c("Isaias Araújo","Marcelo","Matheus Rodrigues","Pablo","Joélio","Raoni")
jogadores_principais=c("Luis Jesiel","Othon Fialho","Gilson Marçal","Gaetan Dubois","Rubens Wanderley","Sérgio Glasherster","Théo Albuquerque")
...
jogos_domino %>% filter(Ano==2018) %>% group_by(Dia_semana) %>%
  ...
  labs(title="Média de Jogos durante a semana em 2018",x="Dia da semana",y="Quantidade")+
  theme(legend.position = "none")#,plot.title = element_text(hjust = 0.5))
...
write.csv2(tabela_mensal,file=paste("ranking_mensal_",month(Sys.Date()),"_",year(Sys.Date()),".csv"))
...
#gráfico de 95% de confiança para o aproveitamento geral dos jogadores
...
ggtitle("Intervalo de Confiança 95% do Aproveitamento")+theme_bw()+xlab("Jogador")+theme(axis.text.x=element_text(angle=90), plot.title = element_text(hjust = 0.5), legend.position = "none")

#intervalo de confiança descartando o mês atual
...
#gráfico do aproveitamento diário
adj1%>% filter(day(DataJogo)>1 & day(DataJogo)<=day(Sys.Date()),AprovAc>0)%>%
  ...
  ggtitle(paste0("Histórico de Aproveitamento Diário - ",month(Sys.Date(), label=T, abbr=F),"/",year(Sys.Date())))+  
...
adj1%>% filter(as.double(pontuacao)>0, day(DataJogo)>1 & day(DataJogo)<=day(Sys.Date()))%>%
  ...
  scale_y_continuous(breaks=seq(0,100,10))+ labs(y="Pontuação")+
  scale_x_date(name="Dias", date_labels = "%d/%m/%Y", date_breaks = "1 day")+
  ggtitle(paste0("Histórico de Pontuação Diária - ",toupper(month(Sys.Date(), label=T, abbr=F)),"/",year(Sys.Date())))+  scale_color_brewer(palette="Set3")+theme_dark()+theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle=90))
...
adj1%>% filter(as.double(pontuacao)>0, day(DataJogo)>1 & day(DataJogo)<=day(Sys.Date()) )%>%
 ...
 #tabelas e gráficos para aproveitamento.
...
#gerar xlsx da média de aproveitamento dos jogadores
...
#histograma dos placares até agora  
