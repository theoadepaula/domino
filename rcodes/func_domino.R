...
tab_mes=function(mes=month(Sys.Date()),ano=year(Sys.Date()),corte=80){
...
graf_mes=function(mes=month(Sys.Date()),ano=year(Sys.Date()),corte=80){
  jogos_domino=read_xlsx("src/Domino.xlsm","Historico")
  jogos_domino=jogos_domino %>% 
    mutate(DataJogo=as.Date(DataJogo),
           Mes=as.integer(month(DataJogo)),
  ...
  dia_inicial=parse_date_time(paste0("1/",mes,"/",ano),order="dmy")
  dia_final=parse_date_time(paste0(days_in_month(mes),"/",mes,"/",ano),order="dmy")
...
knitr::kable(tab_mes()%>%select(-c(Mes,Ano)), caption = paste("Ranking do Mês -",month(Sys.Date(), label=T, abbr=F),"/",year(Sys.Date())),
  ...
  row_spec(which(tab_mes()[,7]<.10), bold = T, color = "purple", background = "green")
